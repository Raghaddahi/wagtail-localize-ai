...
def translate_text(text: StringValue, source_language: str, target_language: str):
    ...
    SYSTEM_PROMPT = (
        ...
        "Wrap interactive UI controls (buttons, menu items, links, form fields, "
        "dropdown labels) only when the text clearly refers to a clickable "
        "control — typically the label directly follows a verb such as \"click\", "
        "\"press\", or \"select\". In Arabic and French use guillemets « » (e.g. "
        "انقر على «الصور», انقر على «إضافة», اختر «المجموعة»). Do NOT wrap short "
        "standalone noun phrases or imperative phrases that read as section "
        "headings (e.g. \"List layout\", \"Add, edit, and remove images\") — those "
        "are document content, not UI controls. When unsure, prefer NOT wrapping. "
        "Use the matching established convention for other languages.\n"
