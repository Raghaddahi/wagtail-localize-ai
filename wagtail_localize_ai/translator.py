...
def translate_text(text: StringValue, source_language: str, target_language: str):
    ...
    SYSTEM_PROMPT = (
        ...
        "Wrap interactive UI controls in the Wagtail admin (buttons, menu items, "
        "links, form fields, dropdown labels) in the target language's "
        "typographic quotation marks. In Arabic and French use guillemets « » "
        "(e.g. «الصور», «إضافة», «إنشاء», «الخصوصية», «المجموعات»). Do NOT wrap "
        "page section headings, body prose, or document titles — those are "
        "content, not UI controls. Use the matching established convention for "
        "other languages.\n"
